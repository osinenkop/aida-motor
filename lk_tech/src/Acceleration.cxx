...
auto Motor::getAcceleration() -> void{
    ...
    this -> acceleration =  (
                            (read_acc_response_[5]      )   +       // *(uint8_t *)(&Accel)
                            (read_acc_response_[6] << 8 )   +       // *((uint8_t *)(&Accel)+1)
                            (read_acc_response_[7] << 16)   +       // *((uint8_t *)(&Accel)+2)
                            (read_acc_response_[8] << 24)           // *((uint8_t *)(&Accel)+3)
                            );
}
...
auto Motor::setAcceleration(uint32_t& acceleration) -> void{
    ...
    this -> write_acc_cmd_[5] = ((acceleration      ) & 0xFF);      // *(uint8_t *)(&Accel)
    this -> write_acc_cmd_[6] = ((acceleration >> 8 ) & 0xFF);      // *((uint8_t *)(&Accel)+1)
    this -> write_acc_cmd_[7] = ((acceleration >> 16) & 0xFF);      // *((uint8_t *)(&Accel)+2)
    this -> write_acc_cmd_[8] = ((acceleration >> 24) & 0xFF);      // *((uint8_t *)(&Accel)+3)

...
SoftwareSerial HoverSerial(13, 15);        // RX, TX
  ...
  // If DEBUG_RX is defined print all incoming bytes
    ...
    if (idx == sizeof(SerialFeedback)) {
        ...
        if (NewFeedback.start == START_FRAME && checksum == NewFeedback.checksum) {
            ...
            Serial.print(String(Feedback.cmd1) + '\n');
            Serial.print(String(Feedback.cmd2) + '\n');
            Serial.print(String(Feedback.speedR_meas) + '\n');
            Serial.print(String(Feedback.speedL_meas) + '\n');
            Serial.print(String(Feedback.batVoltage) + '\n');
            Serial.print( String(Feedback.boardTemp) + '\n');
            Serial.print(String(Feedback.cmdLed) + '\n');
...
void sendStructure(const byte *structurePointer, int structureLength)
{
...
{
  if(Serial.available() < structureLength) return;
...
{ 
  ...
  if (Serial.available() > 0){
    ...
    steer = (int16_t)st.toInt();
    speed = (int16_t)sp.toInt();
  ...
  Send(steer + sin(timeNow), speed + cos(timeNow));
}
